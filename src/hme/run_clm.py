...
class ModelArguments:
    ...
    lora_r: Optional[int] = field(default=-1, metadata={"help": "LoRA attention dimension (rank)."})
    lora_alpha: Optional[int] = field(default=None, metadata={"help": "The alpha parameter for LoRA scaling."})
    ...
    modules_to_save: Optional[str] = field(
        default=None,
        metadata={"help": "Comma-separated list of module names to keep trainable without applying LoRA."},
    )
    merge_when_finished: Optional[bool] = field(
        default=True,
        metadata={"help": "Whether to merge the LoRA adapter into the base model upon completion."},
    )
...
class DataArguments:
    ...
    task_type: str = field(metadata={"help": "The type of task (e.g., 'qa', 'caption', 'pdbbind_reg')."})
...
def prepare_dataset(data_args: DataArguments, is_eval: bool = False) -> HMEDataset:
    ...
    if os.environ.get("LOCAL_RANK") in [None, "0"]:
        ...
        logging.info(f"Dataset length ({'eval' if is_eval else 'train'}): {len(dataset)}")
