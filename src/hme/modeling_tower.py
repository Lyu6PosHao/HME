...
from hme.util import smiles2graph
...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        dtype = x.dtype
        inter = (1 + self.eps) * x + self.propagate(
...
class GNN_graphpred(nn.Module):
    ...
    def forward(self, *argv):
        ...
        graph_representation = None  # self.pool(node_representation, batch) #这里没啥所谓。因为我本来也用不到graph_representation;batch是为了指示哪些node属于同一个图，在单图的情况下，就没用了
...
class Molecule2DTower(nn.Module):
    def __init__(self, config=None, device="cpu"):
        ...
        self.to(self.device)  # 把整个模型搬到device上
        self.eval()
        self.encoder.eval()
    ...
    def forward(self, batch: Union[Dict[str, str], str]) -> torch.Tensor:
        with torch.no_grad():
            ...
            graph = smiles2graph(smiles, self.device)
            _, h_node = self.encoder.encode_mol(
                graph, proj=False, return_node_feats=True
            )
            return h_node
...
class Molecule3DTower(nn.Module):
    ...
    # 此处仅假设x为单个smiles，尽管get_repr支持batch
    def forward(self, batch: Union[Dict[str, List], str, List[str]]) -> np.ndarray:
        with torch.no_grad():
            smiles = batch["smiles"] if isinstance(batch, dict) else batch
            embeddings_3d = self.encoder.get_repr(smiles, return_atomic_reprs=True)[
                "atomic_reprs"
            ]
